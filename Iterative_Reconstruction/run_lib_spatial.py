...
import datasets_spatial as datasets
import evaluation
import likelihood
import sde_lib
  ...
  for step in range(initial_step, num_train_steps + 1):
    ...
    if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
      ...
      if config.training.snapshot_sampling:
        ...
        # kw_real, kw_imag = sample[0,:,:,0], sample[0,:,:,1]
        # kw_complex = kw_real+1j*kw_imag
        # sample = np.log(1+np.abs(kw_complex))
        # image0 = np.fft.ifft2(kw_complex)
        # write_kdata(sample,this_sample_dir,"sample.png")
        # write_data(image0,this_sample_dir,"sample_ifft2.png")
        # #==========================================#
  ...
  for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
    ...
    if config.eval.enable_sampling:
      ...
      for r in range(num_sampling_rounds):
        ...
        kw_real, kw_imag = samples[0,:,:,0], samples[0,:,:,1]
        kw_complex = kw_real+1j*kw_imag
