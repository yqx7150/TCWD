...
import torch
import numpy as np
from sampling2 import NoneCorrector, NonePredictor, shared_corrector_update_fn, shared_predictor_update_fn
import functools
def k2wgt(X,W):
    Y = np.multiply(X,W) 
    return Y
...
def get_pc_inpainter(sde, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_inpainter(score_model, x_input, ori_data, mask,Ksample, weight, Device):
    ...
    with torch.no_grad():
      ...
      x_temp = np.fft.fftshift(np.fft.fft2(ori_data))*mask + x_input* (1. - mask)
      x = np.stack((np.real(x_temp),np.imag(x_temp),np.real(x_temp)))[np.newaxis,:,:]
      ...
      for i in range(sde.N):
        t = timesteps[i]
        x, x_mean = corrector_inpaint_update_fn(model, ori_data, mask, x, t)
        x, x_mean = projector_inpaint_update_fn(model, ori_data, mask, x, t)
